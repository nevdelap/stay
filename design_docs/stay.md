...
- **Alternate-screen takeover** (`ratatui::Terminal` in its default
  ...
  lightweight in-place redraw *within normal scrollback* — it never clears the
  screen, just rewrites a fixed number of lines in place
  ...
  `choose-tree`: the screen clears, the picker owns it fully, and the previous
  terminal content reappears untouched on exit (attach or cancel). This is a
  genuine behavior change from v1 worth remembering when comparing the two side
  by side — not a bug if the picker "looks different," it's the chosen design.
...
- **Typed-ahead input preserved across the picker→attach handoff**: if the user
  ...
  `MenuSelection` into `create_or_attach`.
