...
- Set global options through a server-start config file, not a bootstrap
  ...
  first session's dead pane is retained with its exit status). For an already
  running server, set the same defaults before `new-session` so the requested
  command sees them. `-f` *replaces* `~/.tmux.conf` loading, so the generated
  file must begin with `source-file -q <user config>` to preserve the user's
  ...
  before TASK-057 removed the pattern. Check the explicit `set-option -g` calls'
  status in the already-running-server case. Do not assume options apply
  retroactively.
