...
Issue 1's blue “No README.md!” item is already covered by TASK-036, which
created the top-level README and completed the SIGUSR1 recovery note. The
remaining Issue 1 items are specified below as new tasks.
...
- Make the picker's `r` action safe for terminated sessions: selecting a
  terminated session and pressing `r` must ask for confirmation before killing
  and recreating it. A rejected or cancelled confirmation must leave the
  terminated session untouched.
...
- `src/picker/mod.rs`: add a recreate-confirmation mode using the existing
  `YesNoSelector` pattern. The prompt must name the selected session and offer
  an affirmative and negative choice, with No selected by default. Enter on the
  affirmative choice performs the existing `session::force_recreate_session`
  operation; Enter on No, `n`, or Escape cancels it, while Left/Right only
  changes the selected choice. Do not perform the kill or create call when the
  confirmation is cancelled.
- Keep the current direct recreate behavior for live sessions unchanged. The
  required regression is that a terminated session is never recreated without an
  explicit affirmative confirmation.
- Preserve polling, selection, and action-error behavior after confirmation,
  including errors from the existing shared recreate path.
- Add picker unit/integration coverage for terminated-session `r` entering the
  confirmation, safe cancellation, affirmative recreation, and the unchanged
  live-session path.

Acceptance criteria:

- Pressing `r` on a terminated session renders a confirmation and does not
  change the tmux session until the user confirms Yes.
- No/ Escape leaves the terminated session and its exit status intact.
- Yes invokes the existing force-recreate operation exactly once and refreshes
...
- Make every picker selection reachable and visible when the terminal cannot
  display the complete session list, and visibly indicate when rows exist above
  or below the displayed viewport.
...
- `src/picker/mod.rs`: introduce viewport/scroll state for the list rows. The
  create row and every session row must participate in one logical selection
  sequence; moving with Up/Down must scroll the viewport so the focused row is
  always visible instead of allowing it to fall below the rendered area.
- Reserve a one-column gutter at the right edge of the list viewport. Render a
  grey `↑` in the gutter's first visible row when rows are hidden above and a
  grey `↓` in the gutter's last visible row when rows are hidden below. The
  gutter and markers are never selectable rows, and row text is laid out in the
  remaining width so a marker never overwrites session text or reverse-video
  selection styling.
- Keep the existing content-sized picker and wrapping prompt behavior. Define
  the available list viewport from the actual frame area so the implementation
  works for both alternate-screen and main-screen modes.
- Add deterministic picker render/state tests for top, middle, and bottom
  viewport positions in a terminal shorter than the full list, including
  assertions for the one-column gutter, both marker directions, marker absence
  at the corresponding boundary, and selected-row visibility.

Acceptance criteria:

- Up/Down can reach the create row and every session row in a small terminal.
- The focused row is visible after every selection move.
- The UI indicates hidden rows above and/or below whenever such rows exist, and
