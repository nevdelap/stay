...
- Add an in-picker fuzzy search mode so `/` can narrow a large session list to
  the intended session without creating a new session or requiring the external
  `fzf` executable. The picker must keep the existing attach and navigation flow
  once the matching rows are displayed.

Dependencies:

- None beyond the existing picker implementation. This task is intentionally the
  next task after TASK-081; TASK-082 through TASK-090 retain their stable
  identifiers and remain later plan entries.

Scope:

- `Cargo.toml` and `Cargo.lock`: add the in-process `nucleo` crate at the
  current compatible `0.5` release line. Use its managed worker API from the
  picker; do not invoke an `fzf` subprocess, require an installed command, or
  add a runtime tool dependency. The worker must accept query changes without
  synchronously rescanning the complete inventory on the picker input thread; it
  may retain the previous ranked snapshot internally until the new snapshot is
  ready, but the picker must not expose that stale snapshot as selectable rows.
  Configure matching to be case-insensitive for session names, score fuzzy
  subsequence matches, and order matches by descending matcher score with the
  original tmux inventory order as the deterministic tie-breaker. An empty query
  matches every session in the original inventory order. Do not add a direct
  `nucleo-matcher` dependency or introduce a second custom scoring algorithm.
- `src/picker/mod.rs`: add a filter mode carrying a UTF-8-safe query and cursor
  using the same editing semantics as the existing create and rename prompts. In
  idle mode, `/` must clear transient feedback and pending attach modifiers,
  remember the current selected session (including the unselected synthetic
  create-row state), enter filter mode with an empty query, and select the first
  session row when at least one session exists. The create shortcut and
  Enter-on-create-row behavior remain unchanged outside filter mode.
- In filter mode, derive the visible rows from the complete polled
  `PickerState::sessions` inventory without mutating or losing that inventory.
  Render a `Filter: <query>` input row in the position occupied by
  `create new session`; the create row must not be actionable or visible while
  filtering. Render only matching session rows below it. The first ranked match
  is selected whenever filter mode starts or the query is edited; Up/Down,
  Home/End, and PageUp/PageDown then move among matching session rows only. The
  filter input row is not selectable, so navigation cannot move back to a create
  action. Keep the existing selected-row styling, scrolling, overflow markers,
  session status suffixes, and pending-attach rendering correct for the filtered
  row set.
- Filter mode must explicitly distinguish its no-match state from idle mode's
  synthetic create-row selection: `selected_name = None` while
  `PickerMode::Filter` means that no session is selectable, never that the
  create row is selected. Keep the filter input row fixed and editable at the
  top of the list, keep `list_offset` as an offset into matching session rows
  only, and render a muted non-selectable `No matching sessions` placeholder
  below the input when the match set is empty. The placeholder is never included
  in counts, navigation, selection, or attachment. Enter remains a no-op in this
  state, and Escape still cancels the filter. When the session rows exceed the
  remaining viewport, render the existing overflow markers in the gutter and
  scroll only the session rows; at a narrow width, keep the input row on one
  display line, clip its query around the cursor so the cursor remains visible,
  and never allow the input or placeholder to become an actionable row.
- Tag every worker request and result with a monotonically increasing filter
  session generation, query generation, and inventory generation from which it
  was matched. Increment the filter session generation on every entry into
  filter mode and again when Escape cancels it; increment the query generation
  for every edit and the inventory generation whenever a successful poll changes
  the source inventory. Discard a worker result unless all three generations
  equal the current filter state and filter session. Invalidate or drain the
  worker request when filter mode ends, and ignore all late results from that
  filter session, including after a rapid filter re-entry. While any generation
  is pending, render the fixed filter input plus a muted `Filtering...`
  placeholder, expose no session rows, disable navigation, and make Enter a
  no-op; this prevents a fast Enter from attaching a row ranked for an older
  query or inventory. When the current result is published, replace the
  placeholder atomically, select the first ranked match for a query edit, or
  preserve the selected matching session for a poll when it is still present. A
  worker result for an obsolete query or inventory must never change the visible
  rows or selection.
- Filter editing must accept all printable characters, including `q`, `c`, `r`,
  `e`, `k`, `v`, and `l`, as query text. Support Backspace, Delete, Left/Right,
  Home/End, delete-to-start, delete-to-end, and delete-previous- word with the
  same Unicode scalar and cursor-boundary behavior as the existing name prompts.
  Enter attaches the selected matching session using the existing
  `PickerOutcome::Attach` path; it must never open the create prompt. If there
  are no matches, Enter is a no-op and the status identifies that there are no
  matching sessions. Escape cancels filter mode without attaching or creating:
  restore the pre-filter selected session when it is still present, otherwise
  restore the synthetic create-row selection, clear the query and transient
  filter state, and return to the normal idle status. Once cancelled, `q` again
  quits as it does today.
- Reconcile filtering with polling and layout. A successful inventory poll must
  recompute the ranked visible matches while preserving the currently selected
  matching session by name. If that selected session no longer exists or no
  longer matches, select the first remaining match; if no match remains, clear
  the selection. Clamp `list_offset` against the filtered logical rows and keep
  the preserved selection visible. A transient poll error must retain the last
  complete inventory and its currently visible filtered rows. Idle mode
  continues to calculate selection and scrolling against the existing create row
  plus all sessions. Picker sizing must account for the filter input, query
  width, filtered row names, and filter status text, including narrow terminals
  and Unicode names. Use a concise filter status such as
  `↑/↓ select · Enter attach · Esc cancel`, and retain the existing error
  precedence when a poll or picker action error is present.
- Update `README.md`'s Picker keys table and the finalized picker key/status
  sections in `design_docs/stay.html`, along with picker shortcut/status tests
  and documentation comments, so the idle controls describe `/` as the filter
...
- Add unit coverage for matcher ranking and deterministic ties; empty, exact,
  ...
  attachment; Escape restoration; and inventory polling while filtering. Add a
  ratatui render test proving the filter row replaces the create row and only
  matching sessions are rendered. Add a real-PTY integration test with multiple
  tmux sessions that enters `/`, types a fuzzy query, attaches the selected
  unique match with Enter, detaches back to a fresh picker round, and verifies
  Escape cancels a filter without creating a session. The PTY tests must use the
  existing isolated tmux namespace and cleanup helpers and must enforce
  readiness checkpoints: send `/` alone, wait for a new render containing the
  filter input and filter status, then send the query; wait for a subsequent
  render containing the complete query and the intended ranked match before
  sending navigation or Enter. The Escape-cancellation path must likewise wait
  for the filter-mode render before sending Escape. Add a responsiveness
  regression with a large synthetic inventory proving that query edits enqueue
  worker updates and accept subsequent input without waiting for a complete
  match scan; the test must not rely on a timing-sensitive sleep. Introduce a
  small internal matcher seam: production uses the `nucleo` worker, while a test
  matcher can hold publication behind a barrier or channel and emit
  deterministic `Enqueued(generation)`, `InputHandled`, and
  `Published(generation)` events. The responsiveness test must assert that query
  enqueue occurs before a subsequent input is handled and that result
  publication occurs afterward, without using elapsed-time thresholds. Add a
  delayed-result regression that enters filter mode, cancels it, re-enters
  filter mode immediately, publishes the old session's delayed result, and
  proves it is ignored before publishing and accepting the new session's result.

Acceptance criteria:

- Pressing `/` in the picker visibly replaces `create new session` with the
  filter input, selects the first session match, and does not create or alter
  any tmux session.
- Matching is performed in-process by the managed `nucleo` worker; it is
  case-insensitive, fuzzy, deterministically ranked, and has no dependency on an
  installed `fzf` command or other external executable.
- Typing a query reduces the visible list to the ranked matches, preserves
  Unicode-safe editing, and makes the first ranked match the selected row. With
  one match, Enter attaches directly; with multiple matches, standard navigation
  selects the intended row before Enter attaches; with zero matches, Enter
  cannot create or attach anything.
- With zero matches, the filter row remains visible and editable, the
  `No matching sessions` placeholder is non-selectable, `list_offset` is zero,
  no overflow marker or selected-row highlight is produced for a session, and
  Enter is a no-op. Narrow and overflow layouts retain those same invariants.
- While a current query or inventory result is pending, the filter row remains
  visible and editable, the placeholder reads `Filtering...`, no old session row
  is selectable or attachable, navigation is ignored, and an obsolete worker
  result cannot replace that state. A deterministic unit test edits the query,
  presses Enter before publication, proves no attach occurs, then publishes the
  current generation and proves only the current ranked result can be attached.
- A delayed result from a cancelled filter session is ignored after Escape and
  after immediate filter re-entry, even when its query and inventory generations
  happen to match the new session; only the current filter-session generation
  may update rows or enable attachment.
- Escape restores the pre-filter idle selection and controls, including the
  create row when it was selected before filtering. Filter-mode printable keys
  do not trigger idle shortcuts, and after cancellation `q` still quits.
- Polling, selection clamping, scrolling, narrow layouts, status suffixes, and
  terminal rendering remain correct in both idle and filter modes, with no
  regression to create, rename, attach-modifier, kill, recreate, or quit
  behavior. Polling preserves a still-visible selected match rather than
  resetting navigation on every refresh, and selects the first remaining match
...
- Matching remains responsive with a large inventory: input handling and
  ...
  subsequent input handling, and publication in that order without fixed sleeps.
- The real-PTY tests prove filter-mode entry and ranked-result readiness before
  sending the next input, including the final Enter or Escape action.
- Focused unit, ratatui, and real-PTY tests cover the behaviors above and
  contain no fixed pre-input sleeps. `just qcheck` passes twice after the final
  amend, and the exact `just mac-qcheck` recipe passes.
- Increment the patch version exactly once from the task baseline and update
